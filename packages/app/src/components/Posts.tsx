...
const Post:FC<PostProps> = ({post}) => {
  ...
  <StyledPost>
    ...
    <p>{formattedDate}</p>
    <p>{post.lon} {post.lat}</p>
    <div style={{backgroundColor: 'white', color:'black', margin:"1rem", padding:"1rem", borderRadius:"1rem"}}>
    <p>{post.content}</p>
    </div>
...
export default function Posts() {
    ...
    return (
      <div>
        {data.getAllPosts.map(post => <Post key={post.id} post={post} />)}
      </div>
